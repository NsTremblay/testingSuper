...
The user-interface is created using HTML, CSS and Javascript within the Perl CGI::Application framework. The Phylogenetic tree graphical display will be constructed with the Data Driven Documents (D3) JavaScript library [26] and manipulation of world maps with the Google Maps JavaScript API v3 (https://developers.google.com/maps/documentation/javascript/).

\subsubsection{Server hardware}
SuperPhy is hosted on a server with 16 vCPUs and 96 GB of RAM, running Ubuntu Server 12.04 hosted at the National Microbiology Laboratory in Winnipeg, Canada.
...
2.1. Our previous pan-genomic analyses tool, Panseq [15] will include an iterative analyses mode
that is integrated with the Postgres database, allowing only new genomic sequences to be
analyzed and compared to those already stored in the platform. This computational approach
will allow a continuous influx of new sequence data without large time or memory
requirements. The software will be written in Perl, and will use MUMmer [27] for sequence
comparisons, which relies on suffix-trees for efficient genomic comparisons.
2.2. Phylogenies will be created using Bayesian inference via MrBayes [12]. To deal with the
