...
\abstract{
The advance in DNA sequencing has created new opportunities in fields such as clinical medicine and epidemiology to perform real-time surveillance and analyses of bacterial pathogens. To realize these opportunities, new analytical tools and infrastructure are needed to analyze genomic datasets, store the data, and provide the essential biological information to end-users. We have implemented an online whole-genome analyses platform, with coupled data, called SuperPhy (http://lfz.corefacility.ca/superphy). It comprises 1021 closed and draft \textit{E. coli} genome sequences downloaded from GenBank. To perform genomic comparisons, SuperPhy uses our pan-genomics analysis software Panseq to efficiently perform large-scale genomic comparisons, the Fisher's exact test to identify sub-group specific loci, and FastTree to create maximum-likelihood trees. SuperPhy uses the Chado relational database schema for Postgres 9.2 and facilitates the upload of genomes for both private and public use. Analyses include: 1) time critical genomic comparisons of clinical isolates, identification of virulence and antimicrobial resistance genes \textit{in silico}, 2) associations between specific genotypes and phenotypic metadata (e.g., geospatial distribution, host, source); 3) identification of group-specific genomic markers (presence/absence of specific genomic regions, and single-nucleotide polymorphisms) in bacterial populations, and the ability to manipulate the display of phylogenetic trees and identify statistically significant clade-specific markers.}
...
\begin{figure*}[t]
  \vspace{-0.2cm}
  \centering
   {\includegraphics[width=16cm]{superphy_forms_lores.pdf}}
