...
sub check_group_assignments {
	...
	foreach my $k (@meta_keys) {
		...
		my $other = "$k\_other";
		my $value;
		if($featureprop_hashref->{$k}) {

			my @values = ( ref($featureprop_hashref->{$k}) && ref($featureprop_hashref->{$k}) eq 'ARRAY') ? 
				@{$featureprop_hashref->{$k}} : ($featureprop_hashref->{$k});
	...
	while(my $group_row = $group_rs->next) {
		$assigned_groups{$group_row->feature_group_id} = $group_row->genome_group->standard_value;
	}
