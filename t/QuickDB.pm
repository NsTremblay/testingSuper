...
sub connect {
    ...
    try {
        create_ontology( $schema, 'testdb_cvterms.txt' );
        create_users( $schema );
        create_genome_features( $schema, 'sample1_public_genomes.txt' );
        create_private_genome_features( $schema, 'sample2_public_genomes.txt', 
            &login_crudentials()->{authen_username},
            'public'
        );
        create_private_genome_features( $schema, 'sample3_public_genomes.txt', 
