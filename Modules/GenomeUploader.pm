...
my %featureprop_map = qw(
	serotype           g_serotype
	strain             g_strain
	keywords           g_keywords
	isolation_host     g_source
	isolation_location g_location
	isolation_date     g_date
...
sub edit_genome : Runmode {
    ...
    my $feature_rs = $self->dbixSchema->resultset('PrivateFeature')->search(
    	...
    	{
    		prefetch => [{'private_featureprops' => 'type'}, 'upload', {'dbxref' => 'db'}]
    	}
    );
    ...
    # Set name param
    $t->param(g_name => $feature_row->uniquename);
    
    # Set dbxref params
    if($feature_row->dbxref) {
    	$t->param(g_dbxref_acc => $feature_row->dbxref->accession);
    	$t->param(g_dbxref_ver => $feature_row->dbxref->version);
    	$t->param(g_dbxref_db => $feature_row->dbxref->db->name);
    }
    
    # Set upload params
    $t->param(g_group => $feature_row->upload->tag) if $feature_row->upload->tag && $feature_row->upload->tag ne 'Unclassified';
    if($feature_row->upload->category eq 'release') {
    	$t->param(g_release => 1);
    	$t->param(g_release_date => $feature_row->upload->release_date);
    } elsif($feature_row->upload->category eq 'private') {
    	$t->param(g_private => 1);
    } else {
    	$t->param(g_public => 1);
    }
    ...
    # Featureprops table data that contains simple text fields
    
    
    # Set default value, override later if value found in featureprop table
    $t->param(g_unkn_finished => 1);
    ...
    while(my $featureprop_row = $featureprops_rs->next) {
    	...
    	if($property eq 'mol_type') {
    		# Special case - maps to a radio button
    
    		if($value eq 'wgs') {
    			$t->param(g_wgs_mtype => 1);
    		} elsif($value eq 'chromosome') {
    			$t->param(g_chr_mtype => 1);
    		} elsif($value eq 'genome') {
    			$t->param(g_gen_mtype => 1);
    		} elsif($value eq 'plasmid') {
    			$t->param(g_plas_mtype => 1);
    		} else {
    			$t->param(g_dna_mtype => 1);
    		}
    	} elsif($property eq 'finished') {
    		# Special case - maps to a radio button
    		
    		if($value eq 'yes') {
    			$t->param(g_unkn_finished => 0);
    			$t->param(g_is_finished => 1);
    		} else {
    			$t->param(g_unkn_finished => 0);
    			$t->param(g_not_finished => 1);
    		}
    	} elsif($featureprop_map{$property}) {
    		# Text property, no conversion needed
    		
    		$t->param( $featureprop_map{$property} => $value);
...
sub update_genome : Runmode {
    ...
    my %updated;
    map { $updated{$_}=0} keys %featureprop_map;
    ...
    while(my $featureprop_row = $featureprops_rs->next) {
    	...
    	if($results->valid($featureprop_map{$property})) {
    		
    		# Setting new value
    		get_logger->debug("...set to ".$results->valid($featureprop_map{$property}));
    		$featureprop_row->value($results->valid($featureprop_map{$property}));
    ...
    foreach my $property (keys %featureprop_map) {
    	if($results->valid($featureprop_map{$property}) && !$updated{$property}) {
    		get_logger->debug("Property $property");
    		...
    		$self->dbixSchema->resultset('PrivateFeatureprop')->create(
	    		{
	    			...
	    			upload_id => $upload_id,
	    			value => $results->valid($featureprop_map{$property}),
	    			rank => 0, # New feature, so rank = 0
	    			type_id => $type_row->cvterm_id
	    		}
    		);
    		get_logger->debug("...created with value ".$results->valid($featureprop_map{$property}));
    	}
    }
...
sub _dfv_common_rules {
	...
	return {
		...
		dependencies       => {
			...
			g_location_state => [qw(g_location_country)], # If state is defined, country must also be defined
			g_location_city => [qw(g_location_country g_location_state)], # If city defined, state and country must be defined
