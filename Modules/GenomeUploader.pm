...
sub submit_genome : Runmode {
	...
	$t->param($errs) if $errs;    # created by rm upload_genome
...
sub _dfv_common_rules {
	...
	return {
		required           => [qw(g_name g_source g_date g_location g_strain g_serotype g_mol_type)],
		optional           => [qw(g_description g_keywords g_owner g_synonym g_finished g_release_date g_dbxref_db 
								  g_dbxref_acc g_dbxref_ver g_group)],
		dependency_groups  => {
		...
		dependencies       => {
			...
			inputPrivacy => { 'release', => [qw/g_release_date/]}
		},
		...
		constraint_methods => {
			g_date    => [
				...
				{
					constraint_method => before_today('%F'), # YYYY-MM-DD format
					name => 'before_today'
				}
			],
			g_release_date => [
		...
		msgs => {
			...
			constraints => {
				...
				'valid_fasta_file' => 'Invalid FASTA file',
		...
		eval {
			...
			while(my $seq = $seqio->next_seq) {
				...
				unless($seq->seq =~ m/^[ACGTUNXRYSWKMBDHVacgtunxryswkmbdhv\.-]+$/) {
					get_logger->error("Invalid nucleotide sequence " . $seq->display_id );
				
					if($seq->seq =~ m/([^ACGTUNXRYSWKMBDHVacgtunxryswkmbdhv\.-])/) {
						get_logger->error("Sequence contains invalid nucleotide chars: ",$1);
					}
					
					return;
				}
				get_logger->error("Read sequence: " . $seq->display_id . ", current total length: $total_length");
			}
			
			if($total_length < 1000) {
				get_logger->error("Sequence too short (total length: $total_length)");
			}
